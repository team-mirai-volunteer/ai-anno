# ai-anno
## リポジトリについて
* このリポジトリは、2024年東京都知事選で公開された「[AIあんの](https://note.com/jujunjun110/n/n0362b324831f)」システムをベースに、2025年の活動に向けて開発を進めるプロジェクトです。
* 元のシステムは2024年東京都知事選で安野たかひろ候補が使用したAI応答システムで、マニフェストに基づく質疑応答を行うものでした。
* 2025年では、以下の3つの主要プロジェクトを推進しています：
  * **YouTube生配信PJ**: YouTube Liveでのリアルタイム質疑応答システム
  * **いどばた連携**: いどばたプラットフォームとの連携による大規模意見収集
  * **電話**: 電話による音声質疑応答システム
* 特にRAG（Retrieval Augmented Generation）機能の改善と、音声合成コストの削減、インフラの改善に注力しています。

## アプリケーションの概要
* 事前に政策や考え方を学習させておくことで、YouTube Live、電話、Webインターフェースを通じて、バーチャルアバターや特定の人物の声で視聴者・利用者からの質問に応答できるアプリケーションです
* 2025年参院選に向けたマニフェストベースの質疑応答システムとして活用予定です

## 開発体制・連絡先
* 現在リーダー不在（前任担当者による引き継ぎ中）
* プロジェクト推進：チームみらいボランティア
* 技術的な質問やコントリビュートに関するお問い合わせは、GitHubのIssueまたはPull Requestでお願いします

## ライセンスについて
* このプロジェクトは[GPL-3.0 license](LICENSE)にて公開されています。
* 自治体などの組織が利用する際に調整が必要な場合は、個別にご相談ください。
