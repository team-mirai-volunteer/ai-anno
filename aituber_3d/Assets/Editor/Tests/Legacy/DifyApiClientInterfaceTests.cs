...
namespace AiTuber.Tests.Legacy.Dify
{
    ...
    public class MockDifyApiClient : IDifyApiClient
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var responseText = ShouldReturnError ? _mockErrorMessage : $"{_mockTextResponse}: {request.query}";
            ...
            var result = new DifyProcessingResult
            {
                ...
                TextResponse = ShouldReturnError ? "" : responseText,
                ProcessingTimeMs = _mockDelayMs,
                TotalEventCount = ShouldReturnError ? 0 : 3,
                ErrorMessage = ShouldReturnError ? _mockErrorMessage : null
            };

            if (_mockAudioData != null && !ShouldReturnError)
            {
